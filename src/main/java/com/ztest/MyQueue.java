 ...
 * 我们要实现LinkedBlockingQueue下面下面两个简单的方法put和take put(anObject):把把对象加到BlockingQueue里面，如果BlockQueue没有空间，则调用此方法的线程被阻断
 * 直到BlockingQueue里面没有空间在继续。 take():取走blockingQueue里面该在首位的对象，若blockingQueue为空，阻塞进入等待状态，直到BlockingQueue有新的数据被加入
 *
...
public class MyQueue
{
   ...
   * ，++i和i++操作并不是线程安全的，在使用的时候，不可避免的会用到 synchronized关键字。而AtomicInteger则通过一种线程安全的加减操作接口。
   */
